...
玩4K游戏、进行AI大模型训练或8K视频编辑，都需要大容量显存（>=16GB）。显存不足会导致帧率骤降或任务失败。

- **显存位宽**

GPU与显存之间数据通道的“宽度”，单位是bit。可以理解为高速公路的车道数。

位宽越宽，单位时间内能传输的数据量就越大。它是一个非常关键的、常被忽略的参数。一般为128 / 192 / 256 bit。
...
决定了GPU的适用性。NVIDIA在AI领域凭借CUDA生态占据绝对优势。对于游戏，支持最新的DirectX 12 Ultimate或Vulkan API是必备条件。
...
热设计功耗（TDP），代表 GPU 在最大负载下的功耗设计规范，单位是瓦特。Total Graphics Power（TGP）是指 GPU 实际消耗的功率。

高 TDP 需要更强大的散热系统（更大的散热鳍片、多风扇）。
TDP 也是性能释放的指标，高功耗通常意味着更高的性能上限。
